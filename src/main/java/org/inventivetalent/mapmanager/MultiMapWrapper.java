    ...
    private final MultiMapController controller = new MultiMapController() {
        ...
        public void addViewer(final Player player) {
            if (!viewerIds.contains(player.getUniqueId())) {
                matrixIterator(wrapper -> wrapper.getController().addViewer(player));
        ...
        public void removeViewer(final OfflinePlayer player) {
            matrixIterator(wrapper -> wrapper.getController().removeViewer(player));
            viewerIds.remove(player.getUniqueId());
        }

        @Override
        public void clearViewers() {
            matrixIterator(wrapper -> wrapper.getController().clearViewers());
            viewerIds.clear();
        }
...
//			setContent(split);
        ...
        public void sendContent(final Player player, final boolean withoutQueue) {
            matrixIterator(wrapper -> wrapper.getController().sendContent(player, withoutQueue));
        }
    ...
    public ArrayImage[][] getMultiContent() {
        ...
        for (int x = 0; x < wrapperMatrix.length; x++) {
            if (wrapperMatrix[x].length != wrapperMatrix[0].length) {
    ...
    public void unwrap() {
        matrixIterator(wrapper -> MapManagerPlugin.instance.getMapManager().unwrapImage(wrapper));
    }
    ...
    protected void matrixIterator(MatrixCallable callable) {
        for (MapWrapper[] matrix : wrapperMatrix) {
            for (MapWrapper mapWrapper : matrix) {
                callable.call(mapWrapper);
            }
        }
    }
